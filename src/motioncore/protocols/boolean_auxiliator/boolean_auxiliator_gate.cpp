...
void AssignValues(std::vector<motion::WirePointer>& wires, std::span<const std::uint8_t> s) {
  ...
  for(size_t s = 0; s != bitlen; ++s) {
    auto w = std::dynamic_pointer_cast<boolean_auxiliator::Wire>(wires[s]);
    assert(w);
...
void SetSetupReady(std::vector<motion::WirePointer>& wires) {
  for(auto& w : wires) {
    auto out_wire = std::dynamic_pointer_cast<boolean_auxiliator::Wire>(w);
    assert(out_wire);
...
: Base(backend) {
  ...
  for(size_t s = 0; s != bitlen; ++s) {
    ...
    output_wires_.emplace_back(
      GetRegister().template EmplaceWire<boolean_auxiliator::Wire>(
  ...
  if (my_id != input_owner_id_ && my_id != 0) {
    input_future_ = GetCommunicationLayer().GetMessageManager().RegisterReceive(
        input_owner_id_, communication::MessageType::kBooleanAuxiliatorInputGate, gate_id_);
  }
}
    ...
    case 0: {
      switch (my_id) {
        case 0: {
          ...
          for(size_t s = 0; s != bitlen; ++s) {
            auto out_wire = std::dynamic_pointer_cast<boolean_auxiliator::Wire>(output_wires_[s]);
            assert(out_wire);
...
void InputGate::EvaluateOnline() {
  using namespace std::string_literals;
  using std::to_string;
  using communication::MessageType::kBooleanAuxiliatorInputGate;
  WaitSetup();
  ...
  if(std::size_t(input_owner_id_) == my_id) {
    //Send mx = x ^ lambda
    if (my_id == 0) {
      BroadcastValues(output_wires_, gate_id_, communication_layer, kBooleanAuxiliatorInputGate);
    } else if (my_id == 1) {
      SendValues(output_wires_, 2, gate_id_, communication_layer, kBooleanAuxiliatorInputGate);
    } else if (my_id == 2) {
      SendValues(output_wires_, 1, gate_id_, communication_layer, kBooleanAuxiliatorInputGate);
    }
  } else if(my_id != 0) {
...
boolean_auxiliator::SharePointer InputGate::GetOutputAsBooleanAuxiliatorShare() {
...
: Base(parent->GetBackend()) {
  if (parent->GetProtocol() != MpcProtocol::kBooleanAuxiliator) {
    auto sharing_type = to_string(parent->GetProtocol());
    throw(
        std::runtime_error((fmt::format("BooleanAuxiliator output gate expects a BooleanAuxiliator share, "
                                        "got a share of type {}",
                                        sharing_type))));
  }
  auto my_id = static_cast<std::int64_t>(GetCommunicationLayer().GetMyId());
  ...
  output_wires_.reserve(bitlen);
  for(size_t s = 0; s != bitlen; ++s) {
    auto parent_wire = std::dynamic_pointer_cast<boolean_auxiliator::Wire>(parent_[s]);
    assert(parent_wire);
  ...
  if (output_owner_ == my_id || output_owner_ == kAll) {
    output_future_ = GetCommunicationLayer().GetMessageManager().RegisterReceive(
        (my_id + 1) % 3, communication::MessageType::kBooleanAuxiliatorOutputGate, gate_id_);
  }
}
...
void OutputGate::EvaluateOnline() {
  using namespace std::string_literals;
  using std::to_string;
  using communication::MessageType::kBooleanAuxiliatorOutputGate;
  WaitSetup();
  assert(setup_is_ready_);
  ...
  switch (my_id) {
    case 0: {
      ...
      if (output_owner_ == 2 || output_owner_ == kAll) {
        SendLambdas1(parent_, 2, gate_id_, communication_layer, kBooleanAuxiliatorOutputGate);
      }
      ...
      if(output_owner_ == my_id || output_owner_ == kAll) {
        ...
        for(size_t s = 0; s != bitlen; ++s) {
          auto out_wire = std::dynamic_pointer_cast<boolean_auxiliator::Wire>(output_wires_[s]);
          assert(out_wire);
          auto in_wire = std::dynamic_pointer_cast<boolean_auxiliator::Wire>(parent_[s]);
          assert(in_wire);
    ...
    case 1: {
      ...
      if (output_owner_ == 0 || output_owner_ == kAll) {
        SendValues(parent_, 0, gate_id_, communication_layer, kBooleanAuxiliatorOutputGate);
      }
    ...
    case 2: {
      ...
      if (output_owner_ == 1 || output_owner_ == kAll) {
        SendLambdas2(parent_, 1, gate_id_, communication_layer, kBooleanAuxiliatorOutputGate);
      }
...
boolean_auxiliator::SharePointer OutputGate::GetOutputAsBooleanAuxiliatorShare() {
  return std::make_shared<boolean_auxiliator::Share>(output_wires_);
}
...
: Base(a->GetBackend()) {
  ...
  output_wires_.reserve(bitlen);
  for(size_t s = 0; s != bitlen; ++s) {
    auto a_wire = std::dynamic_pointer_cast<boolean_auxiliator::Wire>(parent_a_[s]);
...
void XorGate::EvaluateSetup() {
  ...
  for(size_t s = 0; s != bitlen; ++s) {
    auto a_wire = std::dynamic_pointer_cast<boolean_auxiliator::Wire>(parent_a_[s]);
    assert(a_wire);
    auto b_wire = std::dynamic_pointer_cast<boolean_auxiliator::Wire>(parent_b_[s]);
    assert(b_wire);
    auto out_wire = std::dynamic_pointer_cast<boolean_auxiliator::Wire>(output_wires_[s]);
    assert(out_wire);
    auto const& a_lambdas1 = a_wire->GetLambdas1();
...
boolean_auxiliator::SharePointer XorGate::GetOutputAsBooleanAuxiliatorShare() {
  return std::make_shared<boolean_auxiliator::Share>(output_wires_);
}
...
: Base(a->GetBackend()),
  triple_{backend_.GetAuxiliatorVerifier()->ReserveTriples64(GetTotalNumberOfBits(a->GetWires()))} {
  
  using communication::MessageType::kBooleanAuxiliatorSetupAndGate;
  using communication::MessageType::kBooleanAuxiliatorOnlineAndGate;
  ...
  if (my_id == 1) {
    multiply_future_online_ = message_manager.RegisterReceive(
        2, kBooleanAuxiliatorOnlineAndGate, gate_id_);
  } else if (my_id == 2) {
    multiply_future_setup_ = message_manager.RegisterReceive(
        0, kBooleanAuxiliatorSetupAndGate, gate_id_);
    multiply_future_online_ = message_manager.RegisterReceive(
        1, kBooleanAuxiliatorOnlineAndGate, gate_id_);
  }
}
...
void AndGate::EvaluateSetup() {
  ...
  switch(my_id) {
    case 0: {
      ...
      auto message = communication::BuildMessage(kBooleanAuxiliatorSetupAndGate, gate_id_, payload);
  ...
  if (my_id != 0) {
    switch (my_id) {
      case 1: {
        ...
        SendValues(output_wires_, 2, gate_id_, communication_layer, kBooleanAuxiliatorOnlineAndGate);
      ...
      case 2: {
        ...
        SendValues(output_wires_, 1, gate_id_, communication_layer, kBooleanAuxiliatorOnlineAndGate);
        const auto message = multiply_future_online_.get();
        const auto payload = communication::GetMessage(message.data())->payload();
        XorAssignValues(output_wires_, {payload->Data(), payload->size()});
        break;
...
boolean_auxiliator::SharePointer AndGate::GetOutputAsBooleanAuxiliatorShare() {
  return std::make_shared<boolean_auxiliator::Share>(output_wires_);
}
